...
DECLARE_MATERIAL_VARIABLE_EX("Emissivw Map Value", _emissive_value, float, 1.0f, 0.0f, 1.0f)
...
VS_OUT std_VS(VS_IN In) 
{
	...
	{
		Out.Color = float4(In.Color, In.Alpha.x) + _diffuse_color;
		Out.HPosition = mul(float4(In.Position.xyz,1), mWorldViewProj);
	}
			...
			VS_OUT In) : COLOR
{
	...
	float fGloss = _use_gloss_map ? InvertValue(tex2D(_glossSampler, _adv_gloss_mat ? adv_coords : def_coords).r, _invert_gloss_map) : 1.0f;
	fGloss *= _gloss_value;

	float fMetal = _use_metal_map ? InvertValue(tex2D(_metalSampler, _adv_metal_mat ? adv_coords : def_coords).r, _invert_metal_map) : 1.0f;
	fMetal *= _metal_value;

	float fEmissive = _use_emissive_map ? InvertValue(tex2D(_emissiveSampler, _adv_emissive_mat ? adv_coords : def_coords).r, _invert_emissive_map) : 1.0f;
	fEmissive *= _emissive_value;

	float fBackscatter = _use_backscatter_map ? tex2D(_backscatterSampler, In.Texcoord.xy).r : 0.0f;
	...
	float3 vDiffuseColor = lerp(vAlbedo.rgb, 0.0f, fMetal);
	float3 vSpecularColor = lerp(0.05f, vAlbedo.rgb, fMetal);

	float3 vSpecularLighting = 0;
	float3 vDiffuseLighting = 0;
	
	DirectionalLight(vNormal, vView, normalize(vLightDirection.xyz), vLightColor, vSpecularColor, fGloss, vDiffuseLighting, vSpecularLighting);
	...
	float3 vFinalColor = vDiffuseLighting * vDiffuseColor + vSpecularLighting + fEmissive * vAlbedo.rgb;
