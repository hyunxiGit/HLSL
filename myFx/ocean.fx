...
int pg(int i)
{
    ...
    int4 t = permutation[y];
    return t[x];
}

int pseudorandom(float x, float y, float z)
{
...
float4 perlin(float4 p , float4 grid)
{
    ...
    aaa = pseudorandom(xi, yi, zi);
    aba = pseudorandom(xi, inc(yi), zi);
    aab = pseudorandom(xi, yi, inc(zi));
    abb = pseudorandom(xi, inc(yi), inc(zi));
    baa = pseudorandom(inc(xi), yi, zi);
    bba = pseudorandom(inc(xi), inc(yi), zi);
    bab = pseudorandom(inc(xi), yi, inc(zi));
    bbb = pseudorandom(inc(xi), inc(yi), inc(zi));
...
float4 PS2(VSout IN) : COLOR
{
    ...
    float4 grid = float4(100.0f, 100.0f, 100.0f, 100.0f);
    float4 col = perlin(IN.pos2, grid);
    return col;
}
